...
pd.Series([2,
           None])  # instead of none automatically displays NaN - not a number, if it can not read a number in clearly otherwise numerical series
...
numbers = numbers.reindex(
    [2, 1, 0, 10])  # reindexes - if the new index value is not in the original series, NaN will appear
...
df = df.append({'numbers': 3, '2': 3.1, '3': 5.5, '4': 4.4, '5th': 3.5, '6th': 3.4},
               ignore_index=1)  # replaces the index!!! and creates a new row if it does not know to which of existing rows to assging the new data
df1 = df.append(
    {'numbers': 3, '2': 3.1, '3': 5.5, '4': 4.4, '5th': 3.5, '6th': 3.4})  # does not know to which row to assign
df1 = df.append(
    pd.DataFrame({'col1': 3, 'col2': 3.1, 'col3': 5.5, 'col4': 4.4, }, index=['row3']))  # remedies the problem above
